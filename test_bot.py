...
async def start_command(client, message):
    ...
    try:
        # 发送测试消息
        await message.reply_text("🚀 测试机器人启动成功！")
        logger.info("✅ /start 命令响应成功")
        
        # 再发送一条消息确认
        await message.reply_text("🎯 这是第二条测试消息")
        logger.info("✅ 第二条消息发送成功")
...
async def ping_command(client, message):
    ...
    try:
        ...
        await reply.edit_text(f"🏓 Pong! 延迟: {latency:.1f}ms")
...
async def handle_all(client, message):
    """处理所有消息"""
    if message.text:
        logger.info(f"💬 收到消息: {message.text[:50]}... 来自用户 {message.from_user.id}")
        
        try:
            await message.reply_text("👋 收到您的消息！发送 /start 开始测试")
...
if __name__ == "__main__":
    logger.info("🎯 测试机器人程序开始...")
    
    try:
        # 运行主函数
        success = asyncio.run(main())
        if success:
            logger.info("✅ 测试机器人运行完成")
        else:
            logger.error("❌ 测试机器人运行失败")
